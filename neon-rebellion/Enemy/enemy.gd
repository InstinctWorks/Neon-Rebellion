	...
	if is_alive:
		...
		if taking_damage:
			...
			if flash_timer <= 0:
				...
				animatedSprite.visible = !animatedSprite.visible  # Flip the Visibility (true/false)
				#update_enemy_visibility()  # Update the Enemy Sprite Visibility
			
			if damage_timer <= 0:
				taking_damage = false
				animatedSprite.visible = true  # Set the Enemy visibility to true
				health_bar.bar_timer.start()
...
func die():
	...
	#set_physics_process(false)  # Pause Enemy Movement
	...
	await get_tree().create_timer(death_length + 0.25).timeout  # Waits until Death animation to finish before proceeding 
